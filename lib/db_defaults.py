...
class dbFunctions:
    ...
    def init_db(self):
        ...
        self.dbcu.execute('CREATE TABLE IF NOT EXISTS multi_disc_strm (m_file TEXT, m_real TEXT, m_title TEXT, m_year TEXT, m_plot TEXT, m_rating TEXT, m_mpaa TEXT, m_dateadded TEXT)')
    ...
    def scn_db(self,updb='false'):
        ...
        return
    def get_dbmd(self,updbmd='false'):
        ...
        if self.json.has_key('result') and self.json['result'].has_key('movies'):
            ...
            # if updb == 'false':
            self.dialpro.create(self.addon.getLocalizedString(30000),self.addon.getLocalizedString(30051))
            self.pc = 1
            for self.item in self.json['result']['movies']:
                self.file = self.item.get('file','')
                self.cfile = os.path.splitext(self.file)[1]
                if self.cfile == ".strm":
                    self.percent = float(self.pc)/float(self.total_m)*100
                    self.m_title = self.item.get('title', '')
                    self.m_year  = self.item.get('year', '')
                    self.m_file  = self.item.get('file', '')
                    # if updb == 'false':
                    self.dialpro.update(int(self.percent),self.addon.getLocalizedString(30051)+"{}".format(self.pc)+self.addon.getLocalizedString(30016)+"{}".format(self.total_m),"{} ({})".format(self.m_title,self.m_year),)
                    self.pc+=1
                    if self.dialpro.iscanceled():
                        # xbmc.executebuiltin('ActivateWindow(Home)')
                        return
                    self.movielist.append(self.item)
            # self.dialog.textviewer("",str(self.movielist))
            return self.movielist
    def scn_dbmd(self,updbmd='false'):
        # self.cln_db(updb)
        self.init_db()
        self.home.setProperty('SF_updateing','true')
        # self.dialog.ok(self.addon.getLocalizedString(30000),self.addon.getLocalizedString(30010))
        self.get_dbmd(updbmd)
        self.total_m=len(self.movielist)
        # self.dialog.ok(self.addon.getLocalizedString(30000),str(self.total_m)+self.addon.getLocalizedString(30011)) 
        # if updb == 'false':
        self.dialpro.create(self.addon.getLocalizedString(30000),self.addon.getLocalizedString(30012)+str(self.total_m))
        ...
        for self.item in self.movielist:
            ...
            self.m_mpaa = self.item.get('mpaa','')
            self.m_dateadded = self.item.get('dateadded','')
            self.real = self.get_path(self.m_file)
            if self.dialpro.iscanceled():
                return
            if self.real != False:
                self.dbcu.execute('SELECT * FROM multi_disc_strm WHERE m_file=?',(self.m_file,))
                self.entry = self.dbcu.fetchone()
                if self.entry is None:
                    self.dbcu.execute('INSERT INTO multi_disc_strm VALUES (?, ?, ?, ?, ?, ?, ?,?)', (self.m_file, self.real, self.m_title, self.m_year, self.m_plot, self.m_rating, self.m_mpaa, self.m_dateadded))
                    self.dbcon.commit()
                    self.dbcu.execute('SELECT * FROM art WHERE m_file=?', (self.m_file,))
                    self.entry = self.dbcu.fetchone()
                    if self.entry is None:
    ...
    def get_path(self,file):
        ...
        for self.line in self.file:
            if xbmcvfs.exists(self.line):
                return self.line
        return False
    def query_pl(self,title,real):
        self.init_db()
        self.dbcu.execute('SELECT * FROM multi_disc_strm WHERE m_title=?', (title,))
        self.entry = self.dbcu.fetchall()
        for self.item in self.entry:
            time.sleep(.5)
            # self.dialog.textviewer("",str(self.item))
            if real == self.item[1]:
                self.m_file=self.item[0]
                self.nitem={'file':self.item[1], 'title':self.item[2], 'year':self.item[3], 'plot':self.item[4], 'rating':self.item[5], 'mpaa':self.item[6], 'dateadded':self.item[7]}
                self.dbcu.execute('SELECT * FROM art WHERE m_file=?', (self.m_file,))
                self.entry = self.dbcu.fetchall()
                for self.item in self.entry:
                ...
                # self.dialog.textviewer("",str(self.nitem))
                return self.nitem
        return
    def log(self,txt):
    ...
    def query_li(self):
        ...
        self.title = xbmc.getInfoLabel("System.CurrentControl")
        self.li_query(self.path,self.title)
        self.dbcon.close()
        return
    def li_query(self,path,title):
        self.init_db()
        self.m_file=path
        self.m_title=title
        # self.dbcu.execute('SELECT * FROM movies WHERE m_title=?', (self.m_title,))
        # more function on widgets
        self.dbcu.execute('SELECT * FROM movies WHERE m_file=?', (self.m_file,))
        self.entry = self.dbcu.fetchall()
        for self.item in self.entry:
            self.m_file = self.item[0]
            self.nitem={'file':self.item[0], 'title':self.item[1], 'year':self.item[2], 'plot':self.item[3], 'rating':self.item[4], 'mpaa':self.item[5], 'dateadded':self.item[6]}
            self.dbcu.execute('SELECT * FROM art WHERE m_file=?', (self.m_file,))
            self.entry = self.dbcu.fetchall()
            for self.item in self.entry:
                self.art={'fanart':self.item[1], 'poster':self.item[2]}
    ...
    def py_mysql(self):
        self.vars()
        self.dbIP = self.addon.getSetting('sqlip')
        # self.dialog.ok("",str(self.dbIP))
        self.port = int(self.addon.getSetting('sqlport'))
        self.user = self.addon.getSetting('sqluser')
        self.password = self.addon.getSetting('sqlpass')
        self.dbName = "LibraryHelper"
        self.charSet     = "utf8mb4"
        self.cusrorType = pymysql.cursors.DictCursor
        self.connectionInstance   = pymysql.connect(host=self.dbIP, port=self.port, user=self.user, password=self.password,charset=self.charSet,cursorclass=self.cusrorType)
        

        try:
            self.cursorInsatnce = self.connectionInstance.cursor()
            self.sqlStatement = "CREATE DATABASE IF NOT EXISTS {}".format(self.dbName)
            self.cursorInsatnce.execute(self.sqlStatement)
            self.sqlQuery = "SHOW DATABASES"
            self.cursorInsatnce.execute(self.sqlQuery)
            self.databaseList = self.cursorInsatnce.fetchall()
            for self.datatbase in self.databaseList:
                self.log(self.datatbase)
        except Exception as e:
            self.log("Exeception occured:{}".format(e))


        self.connectionObject = pymysql.connect(host=self.dbIP,port=self.port, user=self.user, password=self.password, db=self.dbName, charset=self.charSet,cursorclass=self.cusrorType)

        try:

            self.cursorObject = self.connectionObject.cursor()
            self.sqlQuery = "CREATE TABLE IF NOT EXISTS multi_disc_strm (m_file TEXT, m_real TEXT, m_title TEXT, m_year TEXT, m_plot TEXT, m_rating TEXT, m_mpaa TEXT, m_dateadded TEXT)"
            self.cursorObject.execute(self.sqlQuery)
            self.sqlQuery = "show tables"
            self.cursorObject.execute(self.sqlQuery)
            self.rows = self.cursorObject.fetchall()
    ...
    def test(self):
        self.vars()        
        # self.dir = xbmc.translatePath(self.addon.getAddonInfo("profile"))
        self.dir = xbmc.translatePath(os.path.join(self.addon.getAddonInfo("profile"),"settings.xml"))
        self.dialog.textviewer("",str(self.dir))